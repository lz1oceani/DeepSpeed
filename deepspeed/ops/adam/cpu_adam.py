    ...
    def step(self, closure=None, fp16_param_groups=None):
        ...
        for group_id, group in enumerate(self.param_groups):
            for param_id, p in enumerate(group['params']):
                ...
                if not self.fp32_optimizer_states and state['exp_avg'].dtype != torch.float32:
                    ...
                    # assert pid == id(p), "Parameter has changed"
                else:
                    avg_scaling = avg_sq_scaling = new_avg_scaling = new_avg_sq_scaling = 1
                    est_max_avg = est_max_avg_sq = None
                
                # print(p.dtype, est_avg)
                # exit(0)
                """
                # if state['step'] > 1:
                print("Step: ", state['step'])
                print("Max param/grad", torch.max(torch.abs(p.data)).item(), torch.max(torch.abs(p.grad.data)).item())
                print("Actual Max Avg", torch.max(torch.abs(state['exp_avg'])), torch.max(torch.abs(state['exp_avg_sq'])))
                # if state['step'] > 1:
                print(f"Old Parameters: shape={p.data.shape} |  dtype={p.data.dtype} | mean={p.data.float().mean().item()} | max={torch.max(torch.abs(p.data)).item()} | max abs={torch.max(torch.abs(p.data.float())).item()}")
                print(f"Gradient: shape={p.grad.data.shape} | dtype={p.grad.data.dtype} | mean={p.grad.data.float().mean().item()} | max={torch.max(torch.abs(p.grad.data)).item()} | max abs={torch.max(torch.abs(p.grad.data.float())).item()}")
                print(f"Old: mom scaling={avg_scaling} | var scaling={avg_sq_scaling}")
                print(f"New: mom scaling={new_avg_scaling} | var scaling={new_avg_sq_scaling}")
                print(f"Estimated: max mom={est_max_avg} | max var={est_max_avg_sq}")
                """
                ...
                """
                # if state['step'] > 1:
                print(f"New Parameters: shape={p.data.shape} |  dtype={p.data.dtype} | mean={p.data.float().mean().item()} | max={torch.max(torch.abs(p.data)).item()} | max abs={torch.max(torch.abs(p.data.float())).item()}")
                print(f"Actual: max mom={max_avg} | max var={max_avg_sq}")
                print(f"Quantized: max mom={torch.max(torch.abs(state['exp_avg'].float() / new_avg_scaling))} | max var={torch.max(torch.abs(state['exp_avg_sq'].float() / new_avg_sq_scaling))}")
                print(f"Quantized: max mom={torch.abs(state['exp_avg'].float()).max()} | max var={torch.abs(state['exp_avg_sq'].float()).max()}")
                """

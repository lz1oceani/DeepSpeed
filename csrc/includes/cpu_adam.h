                              ...
                              float new_avg_sq_scaling)
{
    ...
    // std::cout << "AVX " << span << " -> avg_scaling: " << avg_scaling << ", avg_sq_scaling: " << avg_sq_scaling << ", new_avg_scaling: " << new_avg_scaling << ", new_avg_sq_scaling: " << new_avg_sq_scaling << std::endl;
    // std::cout << "half_precision_param: " << half_precision_param << ", half_precision_opt_state: " << half_precision_opt_state << std::endl;
    ...
    int AVX_SIZE = SIMD_WIDTH * span;
    // std::cout << "Size & Round Size " << _param_size << " " << new_rounded_size << std::endl;
...
#endif
        // std::cout << "Block Info " << t << " " << copy_size << " " << offset << std::endl;
        // omp_set_num_threads(1);
#pragma omp parallel shared(max_avg, max_avg_sq)
        {
        int idx = omp_get_thread_num();
        // std::cout << "Init value for thread " << idx << " -> " << max_avg[idx] << " " << max_avg_sq[idx] << std::endl;
        // max_avg[idx] = max_avg_sq[idx] = 0.0f;
#pragma omp for
        for (size_t i = t; i < offset; i += SIMD_WIDTH * span) {
            ...
            /*
            std::cout << "AVX Step " << span <<  " [SubBlock " << i << "] -> " << simd_max_abs<span>(momentum_4) << " " << simd_max<span>(variance_4) << std::endl;

            float tmp_avg[AVX_SIZE], tmp_avg_sq[AVX_SIZE];
            simd_store<span>(tmp_avg, momentum_4, false);
            simd_store<span>(tmp_avg_sq, variance_4, false);

            std::cout << "Loaded avg: ";
            for (int j = 0; j < AVX_SIZE; j++) std::cout << tmp_avg[j] << " ";
            std::cout << std::endl;
            std::cout << "===========================" << std::endl;
            std::cout << "Loaded avg sq: ";
            for (int j = 0; j < AVX_SIZE; j++) std::cout << tmp_avg_sq[j] << " ";
            std::cout << std::endl;
            std::cout << "===========================" << std::endl;

            std::cout << "Max AVX " << simd_max_abs<span>(momentum_4) << " " << simd_max<span>(variance_4) << std::endl;

            for (int j = 0; j < AVX_SIZE; j++) 
                tmp_avg[j] = std::abs(tmp_avg[j]);
            std::cout << "Max STL " << *std::max_element(tmp_avg, tmp_avg + AVX_SIZE) << " " << *std::max_element(tmp_avg_sq, tmp_avg_sq + AVX_SIZE) << std::endl;
            */
    ...
    // #pragma omp flush (max_avg, max_avg_sq)

    // for (int i = 0; i < OMP_MAX_THREADS; i++) std::cout << max_avg[i] << " ";
    // std::cout << std::endl;
    
    // for (int i = 0; i < OMP_MAX_THREADS; i++) std::cout << max_avg_sq[i] << " ";
    // std::cout << std::endl;
    

    // auto ret_i =  std::make_pair(*std::max_element(max_avg, max_avg + OMP_MAX_THREADS), *std::max_element(max_avg_sq, max_avg_sq + OMP_MAX_THREADS));
    // std::cout << "AVX Step " << span <<  " [Block " << t << "] -> ret: " << ret_i.first << ", " << ret_i.second << std::endl;
    ...
    *rounded_size = new_rounded_size;
    ...
    // std::cout << "AVX Step " << span <<  " [Final] -> ret: " << ret.first << ", " << ret.second << std::endl;
    return ret;
}
#endif
